...
input.forEach((report) => {
  ...
  for (let i = differenceSet.length - 1; i >= 0; i--) {
    ...
    if (differenceSet[i - 1]) {
      // add the next term in the sequence
      differenceSet[i - 1].push(
        differenceSet[i][differenceSet[i].length - 1] +
          differenceSet[i - 1][differenceSet[i - 1].length - 1]
      );
    }
  }

  // get the last item from the first set of differences and add them to the total

  total += differenceSet[0][differenceSet[0].length - 1];
});
